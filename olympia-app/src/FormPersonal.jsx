...
const FormPersonal = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    telefono: '',
    rol: '2' // Suponiendo ID de Roles: 2=Org Secundario, 3=Asistente
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(res.status === "success") {
        alert("Personal registrado exitosamente y asignado a su rol.");
        setFormData({
  ...
  return (
    ...
    <button
    onClick={() => navigate(-1)}
    className="self-start mb-6 text-blue-600 flex items-center hover:text-orange-800 transition-colors"
    >
    ...
    <h2 className="text-3xl font-bold text-gray-800 mb-2 text-center">Registrar Personal</h2>
    <p className="text-gray-600 mb-8 text-center">Añade organizadores secundarios o asistentes de campo.</p>
    ...
    <div className="pt-4">
    <button type="submit" className="w-full bg-orange-600 text-white font-bold py-3 px-4 rounded hover:bg-orange-700 transition duration-300">
    Guardar Personal
    </button>
    </div>
    </form>
